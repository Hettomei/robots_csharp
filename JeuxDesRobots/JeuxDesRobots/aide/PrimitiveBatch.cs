...
namespace JeuxDesRobots
{
	...
	public class PrimitiveBatch : IDisposable
	{
		...
		/// <summary>
		/// Dessine automatiquement la liste des point envoyé à la couleur spécifiée
		/// </summary>
		/// <param name="listePoint"></param>
		/// <param name="couleur"></param>
		public void dessinerDessinComplet(List<Vector2> listePoint, Color couleur, bool reviensAuPremierPoint)
		{
			this.Begin(PrimitiveType.LineList);


			for (int i = 0; i < listePoint.Count - 1; i++)
			{
				this.AddVertex(listePoint[i], couleur);
				this.AddVertex(listePoint[i + 1], couleur);
			}
			if (reviensAuPremierPoint)
			{
				this.AddVertex(listePoint[listePoint.Count - 1], couleur);
				this.AddVertex(listePoint[0], couleur);
			}
